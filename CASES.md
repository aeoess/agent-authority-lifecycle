...
Cases are grouped by the authority question they ask, not by the research domain they came from. Each case carries exactly one semantic family. The source domain survives as the Domain column in the index below and as the `domain` field in [cases.json](cases.json), and the pre-v0.3 grouping survives there as `fixture_family`. Same family does not mean duplicate: a representative case lists its folded duplicates on a "Variants" line, with the one-line difference that made each a duplicate rather than a separate case.

## Contents

- [Collective and multi-principal authority](#collective-and-multi-principal-authority) (9)
- [Creation and activation](#creation-and-activation) (6)
- [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) (10)
- [Dependencies and subdelegation](#dependencies-and-subdelegation) (6)
- [Evidence, attribution and completeness](#evidence-attribution-and-completeness) (9)
- [External authority-changing events](#external-authority-changing-events) (9)
- [Identity, target and capability drift](#identity-target-and-capability-drift) (8)
- [In-flight actions and authorization boundaries](#in-flight-actions-and-authorization-boundaries) (5)
- [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) (6)
- [Notice, observation and reliance](#notice-observation-and-reliance) (6)
- [Policy version and rule change](#policy-version-and-rule-change) (4)
- [Replication, rollback and stale state](#replication-rollback-and-stale-state) (8)
- [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) (6)
- [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) (11)
- [Scheduled and dormant authority](#scheduled-and-dormant-authority) (5)
...
| ID | Case | Semantic family | Domain | Tier | Fixture |
|---|---|---|---|---|---|
| [LC-A-019](#lc-a-019-co-trustees-act-by-majority-when-they-cannot-reach-unanimity-and-a-vacancy-does-not-have-to-be-filled-before-the-remainder-can-act) | Co-trustees act by majority when they cannot reach unanimity, and a vacancy does not have to be filled before the remainder can act | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | estates-and-trusts | verified | `lifecycle-fiduciary-succession` |
| [LC-C-011](#lc-c-011-a-concurrence-requirement-is-a-gate-at-the-next-authorization-boundary-not-a-second-chain-to-union-with-the-first) | A concurrence requirement is a gate at the next authorization boundary, not a second chain to union with the first | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | military | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-012](#lc-c-012-independently-rooted-chains-can-commit-to-a-shared-objective-without-unioning-their-scopes) | Independently rooted chains can commit to a shared objective without unioning their scopes | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | government | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-016](#lc-c-016-the-bar-for-granting-authority-and-the-bar-for-withdrawing-it-are-not-always-the-same-bar) | The bar for granting authority and the bar for withdrawing it are not always the same bar | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | aviation | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-018](#lc-c-018-concurrence-can-generalize-beyond-a-fixed-pair-to-an-enumerated-role-set-where-silence-itself-blocks-the-action) | Concurrence can generalize beyond a fixed pair to an enumerated role set where silence itself blocks the action | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | medicine | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-029](#lc-c-029-a-shared-coalition-grant-can-carry-different-per-contributor-restrictions-that-a-pooled-ruleset-cannot-represent) | A shared coalition grant can carry different, per-contributor restrictions that a pooled ruleset cannot represent | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | military | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-031](#lc-c-031-a-standing-cause-free-override-can-sit-above-a-concurrently-exercised-authority-without-ever-going-through-a-revocation-channel) | A standing, cause-free override can sit above a concurrently exercised authority without ever going through a revocation channel | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | maritime | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-H-001](#lc-h-001-one-co-settlors-unilateral-revocation-of-a-joint-grant-reaches-only-their-own-contributed-share-not-the-whole-grant) | One co-settlor's unilateral revocation of a joint grant reaches only their own contributed share, not the whole grant | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | estates-and-trusts | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-H-002](#lc-h-002-on-an-account-requiring-multiple-signers-any-one-signers-stop-instruction-controls-over-a-contradictory-pay-instruction-from-a-co-signer) | On an account requiring multiple signers, any one signer's stop instruction controls over a contradictory pay instruction from a co-signer | [Collective and multi-principal authority](#collective-and-multi-principal-authority) | banking-payments | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-A-003](#lc-a-003-an-unmet-contingency-is-indeterminate-not-simply-not-yet-started) | An unmet contingency is indeterminate, not simply "not yet started" | [Creation and activation](#creation-and-activation) | agency-law | verified | `activation-not-established` |
| [LC-C-020](#lc-c-020-an-authority-position-can-be-created-on-the-fly-by-an-eligibility-rule-with-no-pre-named-holder-at-all) | An authority position can be created on the fly by an eligibility rule, with no pre-named holder at all | [Creation and activation](#creation-and-activation) | medicine | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-E-004](#lc-e-004-a-forked-process-inherits-a-snapshot-of-its-parents-authority-not-a-live-reference-to-it) | A forked process inherits a snapshot of its parent's authority, not a live reference to it | [Creation and activation](#creation-and-activation) | platform-engineering | verified | `ancestor-revocation-chain`, `sponsor-handover` |
| [LC-E-006](#lc-e-006-holding-a-scope-does-not-carry-authority-to-hand-it-to-something-you-spawn) | Holding a scope does not carry authority to hand it to something you spawn | [Creation and activation](#creation-and-activation) | platform-engineering | verified | `lifecycle-conferral-without-authority` |
| [LC-E-025](#lc-e-025-a-self-replicated-copy-holds-no-authority-by-default-no-matter-how-identical-it-is-to-an-authorized-original) | A self-replicated copy holds no authority by default, no matter how identical it is to an authorized original | [Creation and activation](#creation-and-activation) | agent-native | verified | `lifecycle-agent-side-events` |
| [LC-H-012](#lc-h-012-a-payment-order-requiring-customer-confirmation-stays-unconfirmed-not-authorized-by-default-when-the-customer-cannot-be-reached-to-complete-the-agreed-procedure) | A payment order requiring customer confirmation stays unconfirmed, not authorized by default, when the customer cannot be reached to complete the agreed procedure | [Creation and activation](#creation-and-activation) | banking-payments | verified | `lifecycle-principal-unreachable` |
| [LC-D-014](#lc-d-014-when-an-issuers-own-issuance-log-cannot-be-trusted-per-artifact-revocation-is-the-wrong-remedy) | When an issuer's own issuance log cannot be trusted, per-artifact revocation is the wrong remedy | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | security-incident | verified | `lifecycle-credential-events` |
| [LC-D-029](#lc-d-029-a-correctly-resolved-historical-key-proves-who-signed-not-what-was-actually-built) | A correctly-resolved historical key proves who signed, not what was actually built | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | security-incident | verified | `lifecycle-credential-events` |
| [LC-D-033](#lc-d-033-adding-a-new-authorized-device-to-an-identity-needs-the-same-scrutiny-as-issuing-a-new-delegation-regardless-of-which-internal-path-added-it) | Adding a new authorized device to an identity needs the same scrutiny as issuing a new delegation, regardless of which internal path added it | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | security-incident | verified | `lifecycle-credential-events` |
| [LC-E-012](#lc-e-012-a-valid-timestamp-alone-does-not-stop-replay-a-maintained-seen-cache-does) | A valid timestamp alone does not stop replay. A maintained seen-cache does | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | verified | `lifecycle-agent-side-events` |
| [LC-F-033](#lc-f-033-systemic-issuer-misbehavior-once-found-escalates-to-distrusting-the-whole-issuer-not-to-revoking-the-chains-that-got-caught) | Systemic issuer misbehavior, once found, escalates to distrusting the whole issuer, not to revoking the chains that got caught | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | verified | `lifecycle-credential-events` |
| [LC-G-001](#lc-g-001-a-planned-key-rotation-at-the-end-of-a-declared-cryptoperiod-is-not-an-investigation-trigger) | A planned key rotation at the end of a declared cryptoperiod is not an investigation trigger | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | verified | `lifecycle-credential-events` |
| [LC-G-002](#lc-g-002-unknown-onset-of-a-key-compromise-forces-the-suspect-window-to-start-at-exposure-not-at-discovery) | Unknown onset of a key compromise forces the suspect window to start at exposure, not at discovery | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | verified | `lifecycle-credential-events` |
| [LC-G-003](#lc-g-003-partitioning-already-issued-artifacts-around-a-signing-key-compromise-needs-an-independent-timestamp-not-the-compromised-keys-own-say-so) | Partitioning already-issued artifacts around a signing-key compromise needs an independent timestamp, not the compromised key's own say-so | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | verified | `lifecycle-credential-events` |
| [LC-G-004](#lc-g-004-a-key-scoped-for-one-purpose-can-be-wrongly-accepted-as-authoritative-for-a-broader-one-if-the-verifier-does-not-check-scope-separately-from-signature-validity) | A key scoped for one purpose can be wrongly accepted as authoritative for a broader one if the verifier does not check scope separately from signature validity | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | verified | `lifecycle-credential-events` |
| [LC-F-035](#lc-f-035-choosing-a-stateless-bearer-credential-format-is-choosing-revocation-only-takes-effect-at-natural-expiry) | Choosing a stateless bearer credential format is choosing "revocation only takes effect at natural expiry" | [Credentials, keys and derived authority](#credentials-keys-and-derived-authority) | cryptography-and-pki | reviewed hypothetical | `lifecycle-credential-events` |
| [LC-B-025](#lc-b-025-a-single-revoked-license-can-invalidate-every-chain-that-depends-on-it-at-once) | A single revoked license can invalidate every chain that depends on it at once | [Dependencies and subdelegation](#dependencies-and-subdelegation) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-D-001](#lc-d-001-a-third-partys-own-breach-disclosure-has-to-become-a-revocation-relevant-trigger-even-though-the-delegating-org-revoked-nothing-itself) | A third party's own breach disclosure has to become a revocation-relevant trigger, even though the delegating org revoked nothing itself | [Dependencies and subdelegation](#dependencies-and-subdelegation) | security-incident | verified | `lifecycle-credential-events` |
| [LC-D-004](#lc-d-004-a-single-compromised-operator-identity-can-be-an-implicit-ancestor-over-many-independent-trees-at-once) | A single compromised operator identity can be an implicit ancestor over many independent trees at once | [Dependencies and subdelegation](#dependencies-and-subdelegation) | security-incident | verified | `lifecycle-credential-events` |
| [LC-H-007](#lc-h-007-checking-each-delegation-artifacts-validity-period-independently-means-a-childs-stated-expiry-can-extend-past-its-parents-without-the-base-algorithm-ever-comparing-the-two) | Checking each delegation artifact's validity period independently means a child's stated expiry can extend past its parent's without the base algorithm ever comparing the two | [Dependencies and subdelegation](#dependencies-and-subdelegation) | cryptography-and-pki | verified | `lifecycle-subdelegation-edges` |
| [LC-H-008](#lc-h-008-delegation-depth-exhaustion-is-a-distinct-failure-mode-rejecting-subdelegation-past-a-chains-own-declared-maximum-depth) | Delegation depth exhaustion is a distinct failure mode, rejecting subdelegation past a chain's own declared maximum depth | [Dependencies and subdelegation](#dependencies-and-subdelegation) | cryptography-and-pki | verified | `lifecycle-subdelegation-edges` |
| [LC-H-009](#lc-h-009-a-child-subdelegation-issued-inside-a-revocations-propagation-window-is-void-once-the-revocation-is-established-a-hypothetical-naming-a-gap-distinct-from-l1) | A child subdelegation issued inside a revocation's propagation window is void once the revocation is established, a hypothetical naming a gap distinct from L1 | [Dependencies and subdelegation](#dependencies-and-subdelegation) | agent-native | candidate | `lifecycle-subdelegation-edges` |
| [LC-B-022](#lc-b-022-a-real-legal-exception-can-supply-exactly-the-basis-a-completeness-claim-needs-without-needing-an-open-ended-we-keep-everything-for-audit-policy) | A real legal exception can supply exactly the basis a completeness claim needs, without needing an open-ended "we keep everything for audit" policy | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-023](#lc-b-023-anticipated-litigation-suspends-routine-record-pruning-before-any-formal-process-is-served) | Anticipated litigation suspends routine record pruning before any formal process is served | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | civil-procedure | verified | none |
| [LC-D-003](#lc-d-003-believed-unused-is-not-the-same-as-verified-inventory-and-the-gap-between-them-is-exactly-where-this-incident-happened) | "Believed unused" is not the same as "verified inventory," and the gap between them is exactly where this incident happened | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | security-incident | verified | `lifecycle-credential-events` |
| [LC-D-025](#lc-d-025-authority-is-currently-valid-and-a-complete-record-exists-of-what-was-done-under-it-are-different-claims-a-verifier-answers-separately) | "Authority is currently valid" and "a complete record exists of what was done under it" are different claims a verifier answers separately | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | security-incident | verified | `lifecycle-credential-events` |
| [LC-F-027](#lc-f-027-an-empty-audit-window-can-mean-the-record-hasnt-arrived-yet-not-that-nothing-happened) | An empty audit window can mean the record hasn't arrived yet, not that nothing happened | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | distributed-systems | verified | `lifecycle-infrastructure-failure` |
| [LC-G-005](#lc-g-005-a-recordkeeping-duty-tied-to-a-now-ended-authority-relationship-outlives-the-relationship-itself-on-its-own-clock) | A recordkeeping duty tied to a now-ended authority relationship outlives the relationship itself, on its own clock | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | regulatory | verified | `lifecycle-evidence-and-record` |
| [LC-I-010](#lc-i-010-a-credential-answering-who-did-this-with-a-group-name-instead-of-a-person-is-a-standard-recognized-failure-mode-not-just-a-bad-habit) | A credential answering "who did this" with a group name instead of a person is a standard-recognized failure mode, not just a bad habit | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | regulatory | verified | `lifecycle-shared-identity-with-no-accountable-principal` |
| [LC-I-011](#lc-i-011-when-an-action-comes-out-of-a-shared-identity-who-is-accountable-can-be-a-fact-that-has-to-be-investigated-after-the-fact-not-something-the-identity-itself-ever-recorded) | When an action comes out of a shared identity, "who is accountable" can be a fact that has to be investigated after the fact, not something the identity itself ever recorded | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | security-incident | verified | `lifecycle-shared-identity-with-no-accountable-principal` |
| [LC-I-012](#lc-i-012-a-privileged-identity-that-can-act-for-everyone-has-to-be-structurally-exceptional-not-just-discouraged-in-policy) | A privileged identity that can act for everyone has to be structurally exceptional, not just discouraged in policy | [Evidence, attribution and completeness](#evidence-attribution-and-completeness) | platform-engineering | verified | `lifecycle-shared-identity-with-no-accountable-principal` |
| [LC-A-001](#lc-a-001-a-principals-death-ends-actual-authority-regardless-of-durability-effective-on-the-agents-notice-not-at-the-instant-of-death) | A principal's death ends actual authority regardless of durability, effective on the agent's notice, not at the instant of death | [External authority-changing events](#external-authority-changing-events) | agency-law | verified | `lifecycle-principal-events` |
| [LC-A-005](#lc-a-005-divorce-automatically-revokes-a-spouses-designation-as-agent-without-any-separate-revocation-act) | Divorce automatically revokes a spouse's designation as agent, without any separate revocation act | [External authority-changing events](#external-authority-changing-events) | agency-law | verified | `lifecycle-principal-events` |
| [LC-A-006](#lc-a-006-the-event-that-revokes-a-spousal-delegation-is-jurisdiction-dependent-filing-not-just-a-final-decree-can-be-the-trigger) | The event that revokes a spousal delegation is jurisdiction-dependent: filing, not just a final decree, can be the trigger | [External authority-changing events](#external-authority-changing-events) | agency-law | verified | `lifecycle-principal-events` |
| [LC-A-012](#lc-a-012-the-coupled-with-an-interest-survival-past-death-rule-is-dictum-the-leading-case-actually-held-the-power-at-issue-died-with-the-principal) | The "coupled with an interest" survival-past-death rule is dictum: the leading case actually held the power at issue died with the principal | [External authority-changing events](#external-authority-changing-events) | agency-law | verified | `lifecycle-principal-events` |
| [LC-B-008](#lc-b-008-authority-can-end-completely-instantly-and-outside-the-delegation-system-entirely) | Authority can end completely, instantly, and outside the delegation system entirely | [External authority-changing events](#external-authority-changing-events) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-009](#lc-b-009-a-legal-event-with-instantaneous-effect-can-disqualify-an-action-before-anyone-could-have-checked-for-it) | A legal event with instantaneous effect can disqualify an action before anyone could have checked for it | [External authority-changing events](#external-authority-changing-events) | insolvency | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-012](#lc-b-012-a-merger-can-vest-authority-in-a-new-principal-by-operation-of-law-with-no-issuance-event-at-all) | A merger can vest authority in a new principal by operation of law, with no issuance event at all | [External authority-changing events](#external-authority-changing-events) | corporate | verified | `lifecycle-organization-events` |
| [LC-B-027](#lc-b-027-forfeiture-changes-who-owns-the-resource-not-just-who-may-deal-with-it-for-now) | Forfeiture changes who owns the resource, not just who may deal with it for now | [External authority-changing events](#external-authority-changing-events) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-031](#lc-b-031-the-same-bankruptcy-chapter-question-can-produce-opposite-authority-outcomes) | The same bankruptcy chapter question can produce opposite authority outcomes | [External authority-changing events](#external-authority-changing-events) | insolvency | verified | `lifecycle-legal-regulatory-events` |
| [LC-D-009](#lc-d-009-a-documented-scope-means-nothing-if-the-resource-side-boundary-never-actually-enforces-it) | A documented scope means nothing if the resource-side boundary never actually enforces it | [Identity, target and capability drift](#identity-target-and-capability-drift) | security-incident | verified | `lifecycle-credential-events` |
| [LC-E-001](#lc-e-001-a-stable-model-alias-is-a-pointer-with-a-documented-default-not-a-pinned-capability-set) | A stable model alias is a pointer with a documented default, not a pinned capability set | [Identity, target and capability drift](#identity-target-and-capability-drift) | agent-native | verified | `capability-binding-drift` |
| [LC-E-002](#lc-e-002-a-valid-non-expired-non-revoked-grant-can-become-unexecutable-and-that-is-a-fourth-state-not-a-variant-of-the-other-three) | A valid, non-expired, non-revoked grant can become unexecutable, and that is a fourth state, not a variant of the other three | [Identity, target and capability drift](#identity-target-and-capability-drift) | agent-native | verified | `lifecycle-agent-side-events` |
| [LC-E-027](#lc-e-027-a-real-precedent-exists-for-gating-expanded-capability-behind-mandatory-re-consent-not-automatic-inheritance) | A real precedent exists for gating expanded capability behind mandatory re-consent, not automatic inheritance | [Identity, target and capability drift](#identity-target-and-capability-drift) | platform-engineering | verified | `lifecycle-agent-side-events` |
| [LC-E-033](#lc-e-033-a-provider-side-capability-upgrade-can-expand-what-a-delegation-authorizes-with-zero-delegation-layer-event) | A provider-side capability upgrade can expand what a delegation authorizes with zero delegation-layer event | [Identity, target and capability drift](#identity-target-and-capability-drift) | agent-native | verified | `capability-binding-drift` |
| [LC-I-001](#lc-i-001-re-registering-an-abandoned-identifier-hands-the-new-controller-everything-still-addressed-to-the-old-one-with-no-delegation-layer-event-marking-the-change-of-hands) | Re-registering an abandoned identifier hands the new controller everything still addressed to the old one, with no delegation-layer event marking the change of hands | [Identity, target and capability drift](#identity-target-and-capability-drift) | naming-and-registry | verified | `lifecycle-identifier-reuse-and-rename` |
| [LC-I-002](#lc-i-002-a-rename-does-not-travel-with-the-references-that-still-point-at-the-old-name-and-a-retirement-policy-meant-to-close-that-gap-can-itself-be-raced) | A rename does not travel with the references that still point at the old name, and a retirement policy meant to close that gap can itself be raced | [Identity, target and capability drift](#identity-target-and-capability-drift) | naming-and-registry | verified | `lifecycle-identifier-reuse-and-rename` |
| [LC-I-003](#lc-i-003-a-recycled-identifier-reused-for-a-new-subject-can-still-unlock-the-old-subjects-authority-because-nothing-marked-the-account-as-depending-on-who-currently-holds-the-number) | A recycled identifier reused for a new subject can still unlock the old subject's authority, because nothing marked the account as depending on who currently holds the number | [Identity, target and capability drift](#identity-target-and-capability-drift) | naming-and-registry | verified | `lifecycle-identifier-reuse-and-rename` |
| [LC-B-028](#lc-b-028-authorization-has-to-be-rechecked-right-up-to-the-moment-of-final-execution-not-just-at-issuance) | Authorization has to be rechecked right up to the moment of final execution, not just at issuance | [In-flight actions and authorization boundaries](#in-flight-actions-and-authorization-boundaries) | banking-payments | verified | `lifecycle-organization-events` |
| [LC-B-029](#lc-b-029-a-receiving-banks-acceptance-is-the-hard-boundary-after-which-a-mid-flight-authority-change-no-longer-stops-the-order) | A receiving bank's acceptance is the hard boundary after which a mid-flight authority change no longer stops the order | [In-flight actions and authorization boundaries](#in-flight-actions-and-authorization-boundaries) | banking-payments | verified | `lifecycle-organization-events` |
| [LC-C-015](#lc-c-015-losing-contact-with-a-directing-principal-should-trigger-a-pre-authorized-fallback-not-a-freeze-or-silent-continuation) | Losing contact with a directing principal should trigger a pre-authorized fallback, not a freeze or silent continuation | [In-flight actions and authorization boundaries](#in-flight-actions-and-authorization-boundaries) | aviation | verified | `lifecycle-time-and-scheduling` |
| [LC-E-013](#lc-e-013-a-long-jobs-own-logic-must-expect-authority-invalidation-to-arrive-silently-mid-run-not-only-at-its-next-gateway-check) | A long job's own logic must expect authority invalidation to arrive silently mid-run, not only at its next gateway check | [In-flight actions and authorization boundaries](#in-flight-actions-and-authorization-boundaries) | cryptography-and-pki | verified | `cached-authorization-revocation` |
| [LC-E-019](#lc-e-019-authority-stays-valid-through-a-scheduled-wind-down-not-cut-off-the-instant-termination-begins) | Authority stays valid through a scheduled wind-down, not cut off the instant termination begins | [In-flight actions and authorization boundaries](#in-flight-actions-and-authorization-boundaries) | platform-engineering | verified | `lifecycle-time-and-scheduling` |
| [LC-A-010](#lc-a-010-a-court-appointed-guardian-or-conservator-can-limit-suspend-or-terminate-an-existing-agents-authority-from-outside-the-delegation-chain-entirely) | A court-appointed guardian or conservator can limit, suspend, or terminate an existing agent's authority from outside the delegation chain entirely | [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) | agency-law | verified | `lifecycle-principal-events` |
| [LC-B-017](#lc-b-017-a-legitimate-new-authority-root-can-come-from-entirely-outside-the-original-chain) | A legitimate new authority root can come from entirely outside the original chain | [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) | corporate | verified | `lifecycle-legal-regulatory-events` |
| [LC-H-004](#lc-h-004-a-boards-suspension-of-a-principals-authority-is-not-valid-board-action-at-all-without-the-quorum-required-for-the-board-to-transact-business) | A board's suspension of a principal's authority is not valid board action at all without the quorum required for the board to transact business | [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) | corporate | verified | `lifecycle-outside-the-chain-standing` |
| [LC-H-005](#lc-h-005-a-labor-boards-reinstatement-order-is-a-new-grant-issued-from-outside-the-original-relationship-not-a-revival-of-the-exact-terminated-one) | A labor board's reinstatement order is a new grant issued from outside the original relationship, not a revival of the exact terminated one | [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) | regulatory | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-H-010](#lc-h-010-a-directors-resignation-is-effective-on-delivery-of-notice-not-on-the-principals-acceptance) | A director's resignation is effective on delivery of notice, not on the principal's acceptance | [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) | corporate | verified | `lifecycle-agent-renunciation` |
| [LC-H-011](#lc-h-011-a-partners-power-to-dissociate-is-effective-immediately-whether-the-dissociation-is-rightful-or-wrongful-and-wrongfulness-is-a-separate-later-liability-question) | A partner's power to dissociate is effective immediately whether the dissociation is rightful or wrongful, and wrongfulness is a separate, later liability question | [Issuer standing and authority to change](#issuer-standing-and-authority-to-change) | corporate | verified | `lifecycle-agent-renunciation` |
| [LC-A-027](#lc-a-027-a-verifiers-internal-chain-is-void-and-a-relying-partys-protected-reliance-are-two-separate-facts-not-one-boolean) | A verifier's internal "chain is void" and a relying party's protected reliance are two separate facts, not one boolean | [Notice, observation and reliance](#notice-observation-and-reliance) | agency-law | verified | `lifecycle-third-party-reliance-notice` |
| [LC-A-028](#lc-a-028-cutting-off-apparent-authority-takes-more-than-one-kind-of-notice-actual-notice-for-known-prior-counterparties-publication-for-strangers) | Cutting off apparent authority takes more than one kind of notice: actual notice for known prior counterparties, publication for strangers | [Notice, observation and reliance](#notice-observation-and-reliance) | agency-law | verified | `lifecycle-third-party-reliance-notice` |
| [LC-C-009](#lc-c-009-a-legitimately-confidential-succession-order-breaks-the-assumption-that-every-chain-is-independently-publicly-verifiable) | A legitimately confidential succession order breaks the assumption that every chain is independently, publicly verifiable | [Notice, observation and reliance](#notice-observation-and-reliance) | government | verified | `activation-not-established` |
| [LC-F-006](#lc-f-006-a-revocation-status-list-served-past-its-own-declared-refresh-time-is-stale-not-confirmed-clean) | A revocation status list served past its own declared refresh time is stale, not confirmed clean | [Notice, observation and reliance](#notice-observation-and-reliance) | cryptography-and-pki | verified | `lifecycle-infrastructure-failure` |
| [LC-F-008](#lc-f-008-a-verifier-that-checks-status-entirely-offline-against-a-synced-snapshot-needs-its-own-evidence-category-not-a-degraded-live-check) | A verifier that checks status entirely offline against a synced snapshot needs its own evidence category, not a degraded live check | [Notice, observation and reliance](#notice-observation-and-reliance) | cryptography-and-pki | verified | `conflicting-status-sources` |
| [LC-B-002](#lc-b-002-two-independently-authoritative-records-of-the-same-fact-can-disagree-and-neither-is-automatically-the-answer) | Two independently authoritative records of the same fact can disagree, and neither is automatically the answer | [Notice, observation and reliance](#notice-observation-and-reliance) | banking-payments | reviewed hypothetical | `conflicting-status-sources` |
| [LC-E-008](#lc-e-008-a-long-running-instances-remaining-steps-must-resolve-against-the-authority-policy-version-it-started-under-not-the-version-deployed-today) | A long-running instance's remaining steps must resolve against the authority-policy version it started under, not the version deployed today | [Policy version and rule change](#policy-version-and-rule-change) | platform-engineering | verified | `lifecycle-time-and-scheduling` |
| [LC-I-004](#lc-i-004-a-policy-rollback-needs-a-defined-current-pointer-not-a-race-to-see-which-edit-lands-last) | A policy rollback needs a defined "current" pointer, not a race to see which edit lands last | [Policy version and rule change](#policy-version-and-rule-change) | platform-engineering | verified | `lifecycle-policy-change` |
| [LC-I-005](#lc-i-005-what-a-past-authorization-decision-is-evidenced-against-has-to-be-the-policy-version-live-at-the-moment-it-happened-not-whatever-version-is-live-when-someone-later-looks) | What a past authorization decision is evidenced against has to be the policy version live at the moment it happened, not whatever version is live when someone later looks | [Policy version and rule change](#policy-version-and-rule-change) | platform-engineering | verified | `lifecycle-policy-change` |
| [LC-I-006](#lc-i-006-when-a-policy-tightens-after-something-was-approved-under-the-old-rule-the-old-grant-doesnt-need-to-be-revoked-and-the-new-rule-doesnt-reach-backward-on-its-own) | When a policy tightens after something was approved under the old rule, the old grant doesn't need to be revoked, and the new rule doesn't reach backward on its own | [Policy version and rule change](#policy-version-and-rule-change) | regulatory | verified | `lifecycle-policy-change` |
| [LC-E-023](#lc-e-023-faithfully-restoring-a-processs-memory-says-nothing-about-whether-the-authority-it-was-holding-is-still-current) | Faithfully restoring a process's memory says nothing about whether the authority it was holding is still current | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | platform-engineering | verified | `authority-epoch-rollback` |
| [LC-E-031](#lc-e-031-a-memory-architecture-built-for-one-continuous-identity-says-nothing-about-whether-its-content-is-current-authority-once-transferred-to-a-different-identity) | A memory architecture built for one continuous identity says nothing about whether its content is current authority once transferred to a different identity | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | agent-native | verified | `lifecycle-agent-side-events` |
| [LC-F-016](#lc-f-016-a-storage-layer-partition-can-silently-resurrect-revoked-authority-below-the-application-layer) | A storage-layer partition can silently resurrect revoked authority below the application layer | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | distributed-systems | verified | `lifecycle-infrastructure-failure` |
| [LC-F-017](#lc-f-017-a-revocation-that-is-true-at-the-source-can-still-read-as-false-at-a-lagging-replica-the-new-enemy-problem) | A revocation that is true at the source can still read as false at a lagging replica: the "new enemy problem" | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | distributed-systems | verified | `lifecycle-infrastructure-failure` |
| [LC-F-018](#lc-f-018-multiple-regional-enforcement-points-are-physically-distinct-copies-of-authority-state-and-can-briefly-disagree) | Multiple regional enforcement points are physically distinct copies of authority state, and can briefly disagree | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | distributed-systems | verified | `conflicting-status-sources` |
| [LC-F-022](#lc-f-022-a-revocation-write-must-not-be-able-to-lose-a-race-to-an-unrelated-lower-stakes-concurrent-write) | A revocation write must not be able to lose a race to an unrelated, lower-stakes concurrent write | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | distributed-systems | verified | `lifecycle-infrastructure-failure` |
| [LC-F-024](#lc-f-024-holding-a-lock-is-not-proof-of-exclusive-access-once-a-pause-can-outlast-the-lease) | Holding a lock is not proof of exclusive access once a pause can outlast the lease | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | distributed-systems | verified | `authority-epoch-rollback` |
| [LC-F-026](#lc-f-026-without-a-quorum-safeguard-an-isolated-primary-keeps-accepting-authority-mutating-writes-nobody-else-will-ever-see) | Without a quorum safeguard, an isolated primary keeps accepting authority-mutating writes nobody else will ever see | [Replication, rollback and stale state](#replication-rollback-and-stale-state) | distributed-systems | verified | `lifecycle-infrastructure-failure` |
| [LC-A-009](#lc-a-009-a-fiduciary-who-feloniously-kills-the-principal-forfeits-the-appointment-retroactively-not-merely-from-the-date-of-the-finding) | A fiduciary who feloniously kills the principal forfeits the appointment retroactively, not merely from the date of the finding | [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) | estates-and-trusts | verified | `lifecycle-principal-events` |
| [LC-A-023](#lc-a-023-ratification-retroactively-authorizes-a-prior-unauthorized-act-but-cannot-be-used-to-defeat-a-third-partys-intervening-rights-and-it-is-all-or-nothing) | Ratification retroactively authorizes a prior unauthorized act, but cannot be used to defeat a third party's intervening rights, and it is all-or-nothing | [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) | agency-law | verified | `lifecycle-fiduciary-succession` |
| [LC-B-007](#lc-b-007-ratification-is-a-new-record-that-reaches-backward-not-a-rewrite-of-the-original) | Ratification is a new record that reaches backward, not a rewrite of the original | [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) | banking-payments | verified | `lifecycle-legal-regulatory-events` |
| [LC-C-006](#lc-c-006-a-tainted-ancestor-invalidates-dependents-only-from-the-moment-the-taint-is-found-not-retroactively) | A tainted ancestor invalidates dependents only from the moment the taint is found, not retroactively | [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) | government | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-022](#lc-c-022-a-pending-ratification-state-is-neither-revoked-nor-not-revoked-and-if-ratification-is-denied-the-record-has-to-say-the-action-was-provisional-throughout-not-that-a-final-revocation-was-later-reversed) | A pending-ratification state is neither "revoked" nor "not revoked," and if ratification is denied, the record has to say the action was provisional throughout, not that a final revocation was later reversed | [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) | military | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-G-006](#lc-g-006-a-decision-receipt-is-evaluated-against-the-view-available-at-decision-time-not-rewritten-by-what-is-learned-afterward) | A decision receipt is evaluated against the view available at decision time, not rewritten by what is learned afterward | [Retroactive findings and recharacterization](#retroactive-findings-and-recharacterization) | government | verified | `lifecycle-evidence-and-record` |
| [LC-A-008](#lc-a-008-purpose-exhaustion-ends-authority-once-an-authenticated-record-shows-the-purpose-was-met-not-once-the-agent-subjectively-decides-it-was) | Purpose exhaustion ends authority once an authenticated record shows the purpose was met, not once the agent subjectively decides it was | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | agency-law | verified | `lifecycle-purpose-exhaustion` |
| [LC-A-033](#lc-a-033-a-newer-delegation-instrument-does-not-automatically-revoke-an-older-one-absent-an-express-revocation-clause) | A newer delegation instrument does not automatically revoke an older one absent an express revocation clause | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | agency-law | verified | `lifecycle-third-party-reliance-notice` |
| [LC-D-010](#lc-d-010-a-credential-with-no-natural-end-issued-for-a-purpose-that-ended-is-a-standing-liability-no-chain-verification-check-can-catch) | A credential with no natural end, issued for a purpose that ended, is a standing liability no chain-verification check can catch | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | security-incident | verified | `lifecycle-purpose-exhaustion` |
| [LC-D-034](#lc-d-034-test-or-legacy-as-a-label-is-not-the-same-as-test-or-legacy-as-a-validity-fact) | "Test" or "legacy" as a label is not the same as "test" or "legacy" as a validity fact | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | security-incident | verified | `lifecycle-purpose-exhaustion` |
| [LC-E-021](#lc-e-021-revoking-a-narrow-lookup-permission-does-not-erase-what-a-session-already-learned-while-that-permission-was-valid) | Revoking a narrow lookup permission does not erase what a session already learned while that permission was valid | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | platform-engineering | verified | `lifecycle-agent-side-events` |
| [LC-H-003](#lc-h-003-bankruptcy-discharge-and-debt-reaffirmation-racing-each-other-resolve-by-a-statutory-sequencing-and-rescission-window-not-a-first-arrival-rule) | Bankruptcy discharge and debt reaffirmation racing each other resolve by a statutory sequencing and rescission window, not a first-arrival rule | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | insolvency | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-I-007](#lc-i-007-reaching-the-stated-end-date-and-being-cut-off-early-for-cause-are-different-events-and-an-evidence-trail-has-to-say-which-one-happened) | Reaching the stated end date and being cut off early for cause are different events, and an evidence trail has to say which one happened | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | regulatory | verified | `lifecycle-expiry-and-renewal` |
| [LC-I-008](#lc-i-008-calling-it-a-renewal-doesnt-make-it-an-extension-some-renewal-processes-issue-an-entirely-new-artifact-and-discard-the-old-one-outright) | Calling it a "renewal" doesn't make it an extension. Some renewal processes issue an entirely new artifact and discard the old one outright | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | cryptography-and-pki | verified | `lifecycle-expiry-and-renewal` |
| [LC-I-013](#lc-i-013-a-grant-issued-for-exactly-one-use-is-supposed-to-stop-being-usable-the-instant-its-used-once-and-a-protocol-can-make-that-a-hard-checked-rule-instead-of-a-convention) | A grant issued for exactly one use is supposed to stop being usable the instant it's used once, and a protocol can make that a hard, checked rule instead of a convention | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | cryptography-and-pki | verified | `approval-single-use`, `lifecycle-purpose-exhaustion` |
| [LC-I-014](#lc-i-014-a-permit-issued-for-a-single-specific-act-can-be-built-so-that-using-it-is-the-same-recorded-event-as-spending-it-with-no-separate-step-required-to-make-it-unusable-again) | A permit issued for a single, specific act can be built so that using it is the same recorded event as spending it, with no separate step required to make it unusable again | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | regulatory | verified | `lifecycle-purpose-exhaustion` |
| [LC-F-009](#lc-f-009-a-publisher-un-saying-its-own-false-revocation-is-a-different-operation-from-reauthorization-and-needs-its-own-evidence-trail) | A publisher un-saying its own false revocation is a different operation from reauthorization, and needs its own evidence trail | [Revocation, expiry and exhaustion](#revocation-expiry-and-exhaustion) | cryptography-and-pki | reviewed hypothetical | `lifecycle-infrastructure-failure` |
| [LC-C-017](#lc-c-017-a-standing-rotation-schedule-is-its-own-form-of-pre-authorization-distinct-from-a-one-time-succession-event) | A standing rotation schedule is its own form of pre-authorization, distinct from a one-time succession event | [Scheduled and dormant authority](#scheduled-and-dormant-authority) | aviation | verified | `lifecycle-time-and-scheduling` |
| [LC-E-014](#lc-e-014-a-recurring-series-can-outlive-the-identity-that-scheduled-it-with-no-automatic-cleanup-and-no-owner-left-to-cancel-it) | A recurring series can outlive the identity that scheduled it, with no automatic cleanup and no owner left to cancel it | [Scheduled and dormant authority](#scheduled-and-dormant-authority) | platform-engineering | verified | `lifecycle-time-and-scheduling` |
| [LC-E-018](#lc-e-018-a-running-scheduled-occurrence-keeps-the-identity-it-was-created-with-only-the-next-occurrence-picks-up-a-tightened-template) | A running scheduled occurrence keeps the identity it was created with. Only the next occurrence picks up a tightened template | [Scheduled and dormant authority](#scheduled-and-dormant-authority) | platform-engineering | verified | `lifecycle-time-and-scheduling` |
| [LC-E-020](#lc-e-020-a-scheduled-jobs-permissions-are-bound-to-its-creators-identity-at-schedule-time-and-the-schedulers-own-docs-are-silent-on-what-happens-once-that-identity-is-gone) | A scheduled job's permissions are bound to its creator's identity at schedule time, and the scheduler's own docs are silent on what happens once that identity is gone | [Scheduled and dormant authority](#scheduled-and-dormant-authority) | platform-engineering | verified | `lifecycle-time-and-scheduling` |
| [LC-E-034](#lc-e-034-a-queued-action-that-outlasts-a-suspension-needs-a-live-check-at-fire-time-not-just-at-the-moment-it-was-queued) | A queued action that outlasts a suspension needs a live check at fire time, not just at the moment it was queued | [Scheduled and dormant authority](#scheduled-and-dormant-authority) | agent-native | reviewed hypothetical | `lifecycle-time-and-scheduling` |
| [LC-A-016](#lc-a-016-some-replacement-authority-is-pre-committed-at-issuance-time-not-issued-fresh-after-the-fact) | Some replacement authority is pre-committed at issuance time, not issued fresh after the fact | [Succession and replacement](#succession-and-replacement) | agency-law | verified | `lifecycle-principal-events` |
| [LC-A-022](#lc-a-022-a-temporary-administrators-authority-is-scoped-to-preservation-and-ends-automatically-the-instant-the-underlying-dispute-resolves) | A temporary administrator's authority is scoped to preservation and ends automatically the instant the underlying dispute resolves | [Succession and replacement](#succession-and-replacement) | estates-and-trusts | verified | `lifecycle-fiduciary-succession` |
| [LC-B-013](#lc-b-013-automatic-corporate-law-vesting-does-not-reach-the-technical-delegation-layer) | Automatic corporate-law vesting does not reach the technical delegation layer | [Succession and replacement](#succession-and-replacement) | corporate | verified | `lifecycle-organization-events` |
| [LC-C-002](#lc-c-002-a-live-capacity-dispute-needs-a-default-holder-a-deadline-and-a-supermajority-override-not-a-block-or-a-race) | A live capacity dispute needs a default holder, a deadline, and a supermajority override, not a block or a race | [Succession and replacement](#succession-and-replacement) | government | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-005](#lc-c-005-two-independently-valid-succession-sources-can-name-different-people-for-the-same-seat-and-nothing-says-which-one-wins) | Two independently valid succession sources can name different people for the same seat, and nothing says which one wins | [Succession and replacement](#succession-and-replacement) | government | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-C-007](#lc-c-007-whether-a-delegation-is-bound-to-an-office-or-to-a-person-is-a-design-choice-the-framework-has-to-represent-explicitly) | Whether a delegation is bound to an office or to a person is a design choice the framework has to represent explicitly | [Succession and replacement](#succession-and-replacement) | government | verified | `lifecycle-root-authority-succession` |
| [LC-C-008](#lc-c-008-a-missing-successor-designation-should-fail-open-to-a-named-statutory-fallback-not-fail-closed-like-an-unknown-revocation) | A missing successor designation should fail open to a named statutory fallback, not fail closed like an unknown revocation | [Succession and replacement](#succession-and-replacement) | government | verified | `lifecycle-time-and-scheduling` |
| [LC-C-014](#lc-c-014-a-handover-can-require-the-incoming-holders-acknowledgment-with-authority-staying-put-until-it-arrives) | A handover can require the incoming holder's acknowledgment, with authority staying put until it arrives | [Succession and replacement](#succession-and-replacement) | aviation | verified | `lifecycle-time-and-scheduling` |
| [LC-C-025](#lc-c-025-continuous-coverage-roles-need-departure-and-successor-designation-to-be-a-single-atomic-operation-not-a-sequence) | Continuous-coverage roles need departure and successor designation to be a single atomic operation, not a sequence | [Succession and replacement](#succession-and-replacement) | industrial-safety | verified | `lifecycle-time-and-scheduling` |
| [LC-D-011](#lc-d-011-inheriting-another-organizations-entire-root-of-trust-by-acquisition-is-not-itself-a-re-verification-event) | Inheriting another organization's entire root of trust by acquisition is not itself a re-verification event | [Succession and replacement](#succession-and-replacement) | security-incident | verified | `lifecycle-credential-events` |
| [LC-H-006](#lc-h-006-a-disputed-root-can-be-deposited-with-a-neutral-forum-pending-resolution-discharging-the-holder-from-choosing-between-claimants) | A disputed root can be deposited with a neutral forum pending resolution, discharging the holder from choosing between claimants | [Succession and replacement](#succession-and-replacement) | civil-procedure | verified | `lifecycle-multiple-principals-and-conflict` |
| [LC-I-009](#lc-i-009-an-interim-holders-mandate-to-keep-the-lights-on-is-not-a-mandate-to-make-new-binding-commitments-in-the-vacancys-name-including-extending-its-own-reach) | An interim holder's mandate to keep the lights on is not a mandate to make new, binding commitments in the vacancy's name, including extending its own reach | [Succession and replacement](#succession-and-replacement) | government | verified | `lifecycle-expiry-and-renewal` |
| [LC-B-004](#lc-b-004-a-successors-fresh-grant-is-bounded-by-the-successors-own-ceiling-not-the-predecessors) | A successor's fresh grant is bounded by the successor's own ceiling, not the predecessor's | [Succession and replacement](#succession-and-replacement) | corporate | reviewed hypothetical | `lifecycle-organization-events` |
| [LC-B-010](#lc-b-010-a-court-supervised-filing-can-add-an-approval-gate-without-pausing-the-underlying-authority) | A court-supervised filing can add an approval gate without pausing the underlying authority | [Suspension, restriction and release](#suspension-restriction-and-release) | insolvency | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-011](#lc-b-011-a-restriction-imposed-by-an-external-filing-needs-an-equally-external-release-trigger) | A restriction imposed by an external filing needs an equally external release trigger | [Suspension, restriction and release](#suspension-restriction-and-release) | insolvency | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-016](#lc-b-016-a-dissolution-statute-can-automatically-narrow-the-scope-of-authority-that-remains-otherwise-valid) | A dissolution statute can automatically narrow the scope of authority that remains otherwise valid | [Suspension, restriction and release](#suspension-restriction-and-release) | corporate | verified | `lifecycle-organization-events` |
| [LC-B-018](#lc-b-018-chain-validity-and-lawfulness-to-execute-are-different-questions) | Chain validity and lawfulness to execute are different questions | [Suspension, restriction and release](#suspension-restriction-and-release) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-019](#lc-b-019-removing-an-external-block-restores-execution-on-an-unbroken-chain-not-through-a-reauthorization) | Removing an external block restores execution on an unbroken chain, not through a reauthorization | [Suspension, restriction and release](#suspension-restriction-and-release) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-024](#lc-b-024-multiple-independent-suspensions-on-the-same-principal-have-to-be-released-independently-not-cleared-by-any-one-of-them-lapsing) | Multiple independent suspensions on the same principal have to be released independently, not cleared by any one of them lapsing | [Suspension, restriction and release](#suspension-restriction-and-release) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-026](#lc-b-026-a-third-party-outside-the-principal-agent-relationship-can-inject-a-new-approval-gate-into-an-otherwise-unmodified-valid-chain) | A third party outside the principal-agent relationship can inject a new approval gate into an otherwise unmodified, valid chain | [Suspension, restriction and release](#suspension-restriction-and-release) | regulatory | verified | `lifecycle-legal-regulatory-events` |
| [LC-B-030](#lc-b-030-a-third-partys-own-contract-can-impose-a-re-authorization-gate-neither-side-of-a-clean-succession-controls) | A third party's own contract can impose a re-authorization gate neither side of a clean succession controls | [Suspension, restriction and release](#suspension-restriction-and-release) | corporate | reviewed hypothetical | `lifecycle-organization-events` |
| [LC-F-013](#lc-f-013-a-clock-skew-rejection-is-a-third-category-neither-expiry-nor-revocation) | A clock-skew rejection is a third category, neither expiry nor revocation | [Time, clocks and validity windows](#time-clocks-and-validity-windows) | cryptography-and-pki | verified | `lifecycle-credential-events` |
| [LC-G-007](#lc-g-007-a-validity-period-check-is-run-against-the-checking-partys-own-clock-at-check-time-not-the-issuers-claimed-clock) | A validity-period check is run against the checking party's own clock at check time, not the issuer's claimed clock | [Time, clocks and validity windows](#time-clocks-and-validity-windows) | cryptography-and-pki | verified | `lifecycle-time-and-scheduling` |
| [LC-G-008](#lc-g-008-a-single-external-time-source-with-no-independent-check-can-silently-go-wrong-and-nothing-downstream-can-detect-it) | A single external time source with no independent check can silently go wrong and nothing downstream can detect it | [Time, clocks and validity windows](#time-clocks-and-validity-windows) | distributed-systems | verified | `lifecycle-time-and-scheduling` |
| [LC-G-009](#lc-g-009-two-legitimately-run-clocks-can-disagree-by-design-during-a-defined-window-and-neither-reading-is-the-wrong-one) | Two legitimately-run clocks can disagree by design during a defined window, and neither reading is the wrong one | [Time, clocks and validity windows](#time-clocks-and-validity-windows) | distributed-systems | verified | `lifecycle-time-and-scheduling` |
| [LC-F-014](#lc-f-014-an-issuers-own-timestamp-is-only-as-trustworthy-as-the-time-source-it-was-signed-against) | An issuer's own timestamp is only as trustworthy as the time source it was signed against | [Time, clocks and validity windows](#time-clocks-and-validity-windows) | cryptography-and-pki | reviewed hypothetical | `lifecycle-infrastructure-failure` |

</details>

## Collective and multi-principal authority

More than one principal is involved. Who has to agree, and whose instruction wins?
...
Variants: LC-A-017 (default solo-authorization rule, joint vs. several, for co-agents acting alone under one instrument, rather than a majority-decision-on-disagreement-plus-vacancy rule for co-trustees).
...
Variants: LC-B-003 (a dual-control money threshold requiring two independent chains before the next authorization boundary, the same "one valid chain is insufficient" gate in a corporate treasury setting rather than a safety-critical one).
...
Variants: LC-A-004 (the verifier's role has to match the specific contingency sub-type claimed, unavailability rather than impairment, not merely be an authorized verifier), LC-C-004 (a conditional, event-extendable expiry window rather than a conditional, unverified activation).
...
Variants: LC-E-007 (a platform default service-linked role lets a spawning agent create a child with no explicit pass grant, the opposite outcome on the same mechanism).
...
Variants: LC-E-026 (a self-replicated swarm modelled as coordinating peers rather than descendants under one root).
...
What does this credential, key or session actually prove, separately from the grant behind it?
...
Variants: LC-F-031 (a transport-layer memory bug leaking active delegation tokens out of process memory, raising the same proactive-rotation-over-the-whole-exposure-window question), LC-F-032 (a multi-tenant proxy bug leaking one tenant's credentials into another tenant's response, the same question at the proxy layer rather than at TLS termination).
...
Variants: LC-E-009 (an approval delivered as 0-RTT-style early data, the same single-use and replay-window gap outside Kerberos).
...
What do the records establish, whom do they attribute an action to, and what set do they cover?
...
Variants: LC-D-028 (a source-code leak surfacing credentials embedded over years, the same no-rotatable-inventory problem reached from the leak side rather than the rotation side).
...
Something happened outside the delegation graph. What did it do to authority, and when?
...
Variants: LC-A-002 (same pattern, a per-delegation flag overriding a default termination rule, but for incapacity and a durability flag rather than death and a coupled-with-interest flag).
...
Variants: LC-C-001 (root-of-chain succession, where no superior issuer could ever exist, tests the same vesting-without-issuance mechanism as a merger vesting a new organizational principal).
...
The name stayed the same while the identity, target or capability behind it changed.
...
Variants: LC-E-003 (deprecation notice windows that differ by model tier, so one countdown assumption fails, rather than the capability jump itself), LC-E-005 (a mutable image tag changing the agent runtime with no logged authority-relevant event, rather than a provider-side capability update).
...
Authority changed between authorization and outcome. What governs at the next authorization boundary?
...
**Fixture:** `cached-authorization-revocation` / `CAR-02`, `CAR-05`, `CAR-06` (merged_candidate, https://github.com/Agent-Authority-Conformance/aps-conformance-suite/blob/d058112c0bf23aeeb35343885d75349100a8ad1e/fixtures/cached-authorization-revocation/README.md)

Variants: LC-E-024 (warm execution-environment reuse letting one invocation's background work bleed into the next, rather than one long job holding a token).
...
Who was entitled to issue, suspend, revoke or replace this, and did they have standing?
...
Who learned of a transition, when, with what freshness, and what may a party relying on the old view still do?
...
Variants: LC-A-029 (adds a statutory presumption of genuineness for good-faith acceptance of an acknowledged instrument, rather than the general common-law notice cutoff), LC-A-032 (a duty to inquire that narrows reliance protection when the transaction is facially inconsistent with the delegation's visible scope, the opposite direction from the general rule).
...
Which copy of authority state is the current one, and what does a restore or a lagging replica bring back?
...
Something established later changes the verdict on a past act. What does that do to the earlier record?
...
Variants: LC-B-032 (an officer suspended pending investigation, where the open part is whether conversion to revocation reaches back to the suspension's start or runs only from the investigation's conclusion, and unsourced where this entry is sourced).
...
Variants: LC-D-012 (the scope of a past disclosure revised upward long afterwards, reaching the same correct-the-record question from the claim side rather than the revocation side).
...
Variants: LC-A-011 (a pre-named successor trustee taking control of a trust that itself persists, rather than a successor-agent list under a power of attorney), LC-C-013 (the pre-committed replacement is a pre-equalized peer inside a fixed two-person crew rather than a named successor).
...
Variants: LC-B-015 (a business-unit spin-off requiring a fresh grant from the new independent entity, rather than an acquisition inheriting an existing root).
...
Authority is paused or narrowed rather than ended. What state is that, and what lifts it?
...
Variants: LC-B-021 (a listing-based restriction that cascades to unlisted affiliates through an ownership fact, rather than applying only to the named entity).
